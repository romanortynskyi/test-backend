...
export class UploadService {
  ...
  _upload(params, progressCallback: (progress: AWS.S3.ManagedUpload.Progress) => void): Promise<any> {
  ...
  async uploadFile(file: Express.Multer.File, progressCallback: (progress: AWS.S3.ManagedUpload.Progress) => void) {
    ...
    try {
      let s3Response = await this._upload(params, progressCallback)
